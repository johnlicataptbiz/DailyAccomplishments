...
python3 "$SCRIPT_DIR/generate_daily_json.py" "$TARGET_DATE" 2>/dev/null || true

# 2) Run integrations but DO NOT push. This prevents drift.
if [ -f "$SCRIPT_DIR/sync_all.py" ]; then
  echo "[$(date)] Syncing integrations (no-push) for $TARGET_DATE..."
  python3 "$SCRIPT_DIR/sync_all.py" "$TARGET_DATE" --no-push 2>/dev/null || true
fi

# 3) Enrichers run AFTER integrations so the committed report includes them
python3 "$SCRIPT_DIR/import_screentime.py" --date "$TARGET_DATE" --update-report --repo "$REPO_ROOT" 2>/dev/null || true

if [ -f "$REPO_ROOT/credentials/calendar.ics" ]; then
  python3 "$SCRIPT_DIR/import_calendar_ics.py" --date "$TARGET_DATE" --ics "$REPO_ROOT/credentials/calendar.ics" --update-report --repo "$REPO_ROOT" 2>/dev/null || true
fi

python3 "$SCRIPT_DIR/import_browser_history.py" --date "$TARGET_DATE" --update-report --repo "$REPO_ROOT" 2>/dev/null || true

# 4) Charts and archives
if python3 -c "import matplotlib" 2>/dev/null; then
  python3 tools/generate_reports.py "$TARGET_DATE" 2>/dev/null || true
fi

"$SCRIPT_DIR"/archive_outputs.sh "$TARGET_DATE" 2>/dev/null || true
